<script lang="ts">
	...
	let activeSubagentStats = $derived.by(() => {
		...
		const latestCached =
			latest?.usage.cachedInputTokens ?? activeSubagent.token?.last?.cachedInputTokens ?? 0;
		const latestFresh = Math.max(0, latestInput - latestCached);
		const latestOutput =
			latest?.usage.outputTokens ?? activeSubagent.token?.last?.outputTokens ?? 0;
		const latestReasoning =
			latest?.usage.reasoningOutputTokens ?? activeSubagent.token?.last?.reasoningOutputTokens ?? 0;
		...
		return {
			...
			stepCount:
				...
				activeSubagent.conversation.reduce(
	...
	{#if subagents.length === 0}
		<div class="rounded-2xl border border-(--line) bg-(--panel) p-10 text-center shadow-xs">
			...
			<p class="mx-auto mt-2 max-w-md text-xs leading-relaxed text-(--muted)">
				This Codex session was executed solely by the main agent thread without delegating to child
				subagents or spawning secondary agent threads.
			</p>
			<div
			...
			>
				...
				<ul class="mt-2 list-disc space-y-1 pl-4">
					<li>
						When Codex executes <code>sub_agent_activity</code> or spawns child threads, they are tracked
						automatically.
					</li>
					<li>
						Each subagent receives segregated telemetry, KPI context saturation, and dedicated
				...
				<div class="flex items-center justify-between text-xs font-semibold">
					<div class="flex items-center gap-2">
						...
						<span class="font-mono text-(--muted)"
							>{numberFormat(fleetStats.mainTokens)} tokens</span
						>
					</div>
					<div class="flex items-center gap-2">
					...
					>
						...
						<div class="mt-4 space-y-2 border-t border-(--line-subtle) pt-3">
							...
							<div class="flex items-center justify-between pt-1">
								...
								<span
									class="rounded border border-(--line) bg-(--field) px-2 py-0.5 font-mono text-[10px] text-(--muted)"
								>
		...
		{#if activeSubagent && activeSubagentStats}
			...
			<div class="space-y-6 rounded-2xl border border-purple-500/30 bg-(--panel) p-5.5 shadow-sm">
				...
				>
					<div class="space-y-1">
						...
						<div class="flex flex-wrap items-center gap-3 font-mono text-xs text-(--muted)">
							...
							{#if sub.cwd}
								<span>·</span>
								<span class="max-w-xs truncate">{sub.cwd}</span>
							{/if}
						</div>
					</div>
				...
				<div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
					...
					<div
						class="space-y-2 rounded-xl border border-(--line) bg-(--panel-subtle)/50 p-4 shadow-xs"
					>
						...
						<div class="flex items-baseline justify-between">
							...
							<span class="font-mono text-xs text-(--muted)"
								>/ {numberFormat(stats.maxContext)}</span
							>
						</div>
				...
				<div class="space-y-3">
					...
					{#if !sub.conversation || sub.conversation.length === 0}
						<div
							class="rounded-xl border border-(--line) bg-(--field) p-8 text-center text-xs text-(--muted)"
						>
												...
												{#if expandedSubagentActivities.has(msg.id)}
													<div class="space-y-2 p-2">
														{#each msg.activity.tools as tool (tool.id)}
															<div
																class="rounded-lg border border-(--line-subtle) bg-(--field) p-2.5 font-mono text-xs"
															>
																<div class="flex items-center justify-between text-[11px]">
																	<div class="flex items-center gap-1.5">
																		...
																		{#if tool.status}
																			<span
																				class="py-0.2 rounded bg-purple-500/20 px-1.5 text-[10px] text-purple-300"
																			>
																				{tool.status}
																			</span>
																		{/if}
																...
																{#if tool.input}
																	<div class="mt-2 space-y-1">
																		...
																		>
																		<pre
																			class="max-h-36 overflow-x-auto rounded bg-(--canvas) p-2 text-[11px] whitespace-pre-wrap text-(--ink)">{tool.input}</pre>
																	</div>
																{/if}
																{#if tool.output}
																	<div class="mt-2 space-y-1">
																		...
																		>
																		<pre
																			class="max-h-48 overflow-x-auto rounded bg-(--canvas) p-2 text-[11px] whitespace-pre-wrap text-(--ink)">{tool.output}</pre>
																	</div>
																{/if}
															</div>
														{/each}
